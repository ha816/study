...
디플로이먼트의 YAML의 파일에는 단지 포드의 애플리케이션이 사용할 내부 포트만 정의합니다. containerPort항목이 바로 내부에서 사용할 포트번호 입니다. 내부 포트만 정의해서 이 포드가 바로 외부로 노출되는 것은 아닙니다. 이 **포트를 외부로 노출해 사용자들이 접근하거나, 다른 디플로이먼트의 포드들이 내부적으로 접근하려면 서비스**라 불리는 별도의 쿠버네티스 오브젝트를 사용해야 합니다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTM1NzgzNjkyLC0xMDAwMDQwNjk2XX0=
-->