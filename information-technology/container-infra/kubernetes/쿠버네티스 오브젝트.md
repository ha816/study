...
Cluster는 기본적으로 NodePort와 LoadBalancer가 사용하는 방식입니다. 이를 Local로 설정하면 요청 전달시 **포드가 생성된 노드에서만 포드로 접근할 수 있습니다.** 로컬 노드에 위차한 포드 중 하나로만 요청이 전달됩니다. 즉 추가적인 네트워크 홉이 발생하지 않습니다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIxNjcwMzgzOCwyOTQyNDQ0MzYsLTEyOT
