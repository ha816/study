...
한 프로젝트에서 여러 종류의 테스트를 정의하고 실행하는 것은 자주 있는 일입니다. (unit tests, integration tests과 같은) 각 타입의 테스트 소스 코드는 소스 디렉토리에 국한된 곳에 저장되어야 좋습니다. 이렇게 나눠진 테스트 코드는 각 테스트 타입을 독립적으로 수행할 수 있어 유지보수와 관심사의 분리에 좋습니다. 

아래 소스 트리는 integration test를 어떻게 나누는지 보여줍니다.
...
```
source sets은 소스 코드를 컴파일하는데 까지만 관여합니다. byte code를 실행하는 것을 다루진 않습니다. integTest의 테스트 수행을 위해서, Test 타입에 task가 정의 되어야 합니다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwMjcwNDk1OTAsMTQ3NDc2NzY3MiwtMT
