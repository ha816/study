...
저널로그뿐만 아니라 디스크 데이터 까지 모두 동기화하고 클라이언트에게 성공/실패 여부를 반환합니다. 파일을 통째로 디스크로 동기화하는 것은 높은 비용이지만 저널로그가 없던 시점에는 많이 사용 되었습니다.  하지만 저널 로그가 생기고 나선 거의 ㅈ
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTcwNDY0NzEzOSwtMzY0ODY0NDUyLDEzMz
