...
objectid값이 시간에 따라 커져야 하는 것은 맞지만, 필수불가결하게 단조롭게 모든 objectid값이 항상 커지는 것은 아니다. 왜냐하면 같은 초에 생성된 값은 순서를 보장하지 않고  
    
    -   Only contain one second of temporal resolution, so  [ObjectId](https://docs.mongodb.com/manual/reference/bson-types/#objectid)  values created within the same second do not have a guaranteed ordering, and
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE1Mzc4MDU5NTIsLTEyMTk5MTk4MjQsOD
