...
로컬 메모리 영역(Local Memory Area, Session Memory Area)
: 클라이언트 쓰레드가 사용하는 대표적인 메모리 영역이다. 클라이언트 쓰레드가 사용하는 메모리 공간이라고 해서 클라이언트 메모리 영역이라고도 한다. 또 서버
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjA5NDQ3NTMxOCwzMzYyNjQyMzgsMTg5Mj
