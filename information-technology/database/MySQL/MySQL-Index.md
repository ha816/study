...
만약 인덱스의 키 값의 평균 크기가 늘어나면 어떤 현상이 추가로 더 발생하는지 알아보자. 만약 B-Tree의 깊이가 3인 경우 최대 몇 개의 페이지를 가질수 있을까?
...
![enter image description here](http://wiki.gurubee.net/download/attachments/6816042/index_range_scan.PNG)
...
**사실 빠른 검색의 전제 조건은 바로 정렬이다.** 컬럼으로 검색해도 값의 왼쪽 부분이 없으면 인덱스 레인지 스캔이 불가능하다. 그런 이유로 아래 쿼리는 레인지 스캔 방식을 이용할 수 없다.
