...
UPDATE 문장이 실행되면 커밋 실행 여부와 관계 없이, Buffer Pool이 새로운 값으로 수정되고, 디스크의 데이터도 일반적으로 동일한 상태의 데이터가 된다.  그러면 아직 COMMIT이나 ROLLBACK이 일어나지 않은 상태에서 다른 사용자가 작업 중이던 레코드를 조회하면 어떤 버전의 데이터를 조회할까??
...
# MySQL 로그 파일
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE4MjQ0NDI0MjcsMTQxMzgzMzc3OCwtMT
