...
그렇다면 그 방대한 릴레이션 테이블을 엘라스틱 서치에서는 어떻게 처리할까? 답은 간단하다. 하나의 인덱스는 하나의 타입만 가져야 하니, 그저 다수의 인덱스를 만들면 된다. 실제 엘라스틱서치는 다수의 인덱스를 검색에 활용하고  여러 물리적인 서버에 

하나의 물리적인 노드에 여러 개의 논리적인 인덱스를 생성할 수 있다. 검색 시 인덱스 이름으로 문서 데이터를 검색하며, 여러 개의 인덱스를 동시에 검색하는 것도 가능하다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE1MDAwMzk3NTldfQ==
-->