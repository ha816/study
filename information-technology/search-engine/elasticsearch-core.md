...
검색 시스템에는 엄청나게 많은 문서가 저장될 수 있다. 하나의 하드웨어에서 제공하는 리소스의 물리적인 한계를 넘기 위해 도입한 개념이 바로 샤드이다. 이를 이용하면 데이터를 분삭 저장할때 손쉽게 수평확장이 가능해진다. 


인덱스 내부에 색인된 문서들은 여러 파티션으로 나뉘어 구성되는데, 이 파티션을 엘라스틱 서치에서는 샤드(Shard)라 부른다. 엘라스틱 서치는 다수의 샤드로 문서를 분산 저장하고 있어 데이터 손실 위험을 줄일 수 있다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNzA2ODIzNzg5LDE0MTYyNTM0NV19
-->