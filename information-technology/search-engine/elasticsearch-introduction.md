...
**엘라스틱서치도 NoSQL의 일종으로 분산 처리를 통해 실시간에 준하는 빠른 검색이 가능하다.** 기존 데이터 베이스에서는 처리하기 어려운 비정형의 데이터도 검색할 수 있으며, 전문 검색과 구조 검색 모두를 지원한다. 기본적으로 검색 엔진이지만 MongoDB나 Hbase처럼 대용량 스토리지로도 활용할 수 있다. 
