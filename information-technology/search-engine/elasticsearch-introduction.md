...
구글이나 네이버에서 제공하는 서비스 중 빼놓을 수 없는 서비스는 바로 검색 서비스이다. 이 검색 서비스를 부르는 용어도 다양한데, 검색 엔진, 검색 시스템, 검색 서비스 등의 용어가 대표적이다. 

대다수의 독자는 위 세 가지 용어가 비슷하다고 생각할 것이다. 하지만 차이가 있으며 이를 한번 집고 넘어가자
...
검색 엔진을 설명할때 항상 비교되어 설명되는 것이 관계형 데이터베이스(RDBMS)이다. 두 가지 모두 질의와 일치하는 데이터를 찾아 사용자에게 제공한다는 점에서 유사점이 많다. 하지만 관계형 데이터 베이스만으로는 검색 기능에 한계가 있다.

RDBMS에서 모든 데이터는 정형 데이터로 구조화되는데 행과 열로 구성된 테이블이 그것이다. SQL문을 이용한 정보 검색은 텍스트 매칭을 통한 단순 검색만이 가능하다. 텍스트를 다른 단어로 변형하거나 여러 개의 동의어나 유의어를 활용한 검색은 불가능하다. 
...
위 차트는 엘라스틱 서치와 관계형 데이터베이스의 주요 개념을 배교해서 보여준다. 

## 검색 시스템으로서 엘라스틱서치

대량의 데이터를 빠르게 검색하기 위해 고안된 NoSQL(No Structured Query Language)을 검색 시스템으로 많이 사용한다.

**엘라스틱서치는 루씬 라이브러리 기반의 오픈소스 검색엔진이다. 엘라스틱서치는 NoSQL의 일종이고 분산 처리를 통해 실시간에 준하는(Near-Real-Time) 검색이 가능하다.** 기존 데이터 베이스에서는 처리하기 어려운 비정형의 데이터도 검색할 수 있으며, 전문 검색과 구조 검색 모두를 지원한다. 기본적으로 검색 엔진이지만 MongoDB나 Hbase처럼 대용량 스토리지로도 활용할 수 있다. 
...
* 역색인(Inverted Index)
	* 엘라스틱 서치는 루씬 기반의 검색엔진이다. 따라서 엘라스틱서치 또한 역색인을 지원한다. 반면 MongoDB나 카산트라(Casandra)같은 일반적인 NoSQL은 역색인을 지원하지 않는다. 이는 다른 NoSQL 대비 엘라스틱서치의 큰 장점이다. 

### 엘라스틱서치 단점

* 완벽한 실시간 검색이 아니다. 
	* 일반적으로 색인된 데이터는 통상적으로 1초 뒤에나 검색이 가능하다. 색인된 데이터는 내부적으로 커밋과 플러시 같은 복잡한 과정을 거치기 때문에 실시간이 아니다. 엄밀히 말하면 근실시간(Near-Real-Time)이라 할 수 있다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwMDc2NjI3OTAsMjA0OTY5NjYzMl19
-->