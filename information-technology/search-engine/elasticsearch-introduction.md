...
**엘라스틱서치는 루씬 라이브러리 기반의 오픈소스 검색엔진이다. 엘라스틱서치는 NoSQL의 일종이고 분산 처리를 통해 실시간에 준하는(Near-Real-Time) 검색이 가능하다.** 기존 데이터 베이스에서는 처리하기 어려운 비정형의 데이터도 검색할 수 있으며, 전문 검색과 구조 검색 모두를 지원한다. 기본적으로 검색 엔진이지만 MongoDB나 Hbase처럼 대용량 스토리지로도 활용할 수 있다. 엘라스틱 서치의 장점은 아래와 같다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwNTAyNDUxMTksMjA0OTY5NjYzMl19
-->