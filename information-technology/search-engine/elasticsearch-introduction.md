...
구글이나 네이버에서 제공하는 다양한 서비스 중는 빼놓을 수 없는 서비스는 바로 검색포털 사이트 서비스이다. 이 검색 서비스를 부르는 용어도 다양한데, 검색 엔진, 검색 시스템, 검색 서비스 등의 용어가 대표적이다. 

대다수의 독자는 위 세 가지 용어가 비슷하다고 생각할 것이다. 하지만 차이가 있으며 이를 한번 집고 넘어가자세 가지 용어는 명백히 다르다. 

검색 엔진(search engine)
: 광활한 웹에서 정보를 수집해 검색 결과를 제공하는 프로그램이다.

검색 시스템(search system)
: 대용량 데이터를 기반으로 뛰어난신뢰성 있는 검색 서비스결과를 제공하기 위해 다수의 검색 엔진을 활용하는기반으로 구축된 시스템을 통칭하는 용어이다. 

검색 서비스(search service)
: 구축한 검색 시스템으로 실제 검색검색엔진을 기반으로 구축한 검색 시스템을 활용해 검색 결과를 서비스를로 제공한다. 
...
RDBMS에서는 모든 데이터는 중복을 제거하고 정형의 데이터로 구조화되는데해서 행과 열로 구성된 테이블이 그것이로 저장된다. SQL문을 이용한 정보 검색은 텍스트 매칭을 통한 단순한 검색만이 가능하다. 텍스트를 다른여러 단어로 변형하거나 여러 개의 동의어나 유의어를 활용한 검색은을 불가능하다. 

반면에 검색엔진(search engine)은 데이터베이스에서는 불가능한 비정형 데이터를 색인하고 검색할 수 있다. 형태소 분석을 통해 사람이 구사하는 자연어의 처리가 가능해지고 역색인 구조를 바탕으로 빠른 검색 속도를 보장한다. 
...
엘라스틱서치는 대량의 데이터를 빠르게 검색하기 위해 고안된 NoSQL(No Structured Query Language)을 검색 시스템으로 많이 사용한다.

**엘라스틱서치는 루씬 라이브러리 기반의 오픈소스 검색엔진이를 많이 사용한다. 엘라스틱 서치는도 NoSQL의 일종이고으로 분산 처리를 통해 실시간에 준하는(Near-Real-Time) 빠른 검색이 가능하다.** 기존 데이터 베이스에서는 처리하기 어려운 비정형의 데이터도 검색할 수 있으며, 전문 검색과 구조 검색 모두를 지원한다. 기본적으로 검색 엔진이지만 MongoDB나 Hbase처럼 대용량 스토리지로도 활용할 수 있다. 
...
* 역색인(Inverted Index)
	* 엘라스틱 서치는 루씬 기반의 검색엔진이다. 따라서 엘라스틱서치 또한 역색인을 지원한다. 반바면 MongoDB나 카산트라(Casandra)같은 일반적인 NoSQL은 역색인을 지원하지 않는다. 이는 다른 NoSQL 대비 엘라스틱서치의 매우 큰 장점이다. 
...
* 실시간 검색이 아니다. 
	* 일반적으로 색인된 데이터는 통상적으로 1초 뒤에나 검색이 가능하다. 색인된 데이터는 내부적으로 커밋과 플러시 같은 복잡한 과정을 거치기 때문에 실시간이 아니다. 엄밀히 말하면 근실시간(Near- Real-Ttime)이라 할 수 있다.
* 트랜잭션과 롤백 기능을 제공하지 않는다. 
	* 엘라스틱 서치는 기본적으로 분산 시스템으로 구성된다. 전체적인 클러스터의 성능을 위해 비용 소모가 큰 롤백(Rollback)과 트랜잭션을 지원하지 않기 때문에 최악의 경우 데이터 손실의 위험이 있다. 
* 데이터의 수정 기능을업데이트를 제공하지 않는다. 
	* 엄밀히 말하면 업데이트 명령이 요청될 경우 기존 문서를 삭제하고 새로운 내용으로 문서를 생성하는 방식을 사용한다. 따라서 단순 업데이트에 비해서는 많은 비용이 발생한다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE5NDI4Mjk1NjAsLTE3NjAwNTczNTgsMj
A3NzMzNjAyNSw0Njg2NzMzMjIsMjA0OTY5NjYzMl19
-->