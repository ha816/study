...
샤드에는 두 종류가 존재한다. 첫 번째는 실질적인 CRUD 서비스를 제공하는 **프라이머리 샤드로 엘라스틱서치에서 핵심요소이다. 프라이머리 샤드를 일반적으로 샤드로 부른다.** 두 번째는 레플리카 샤드다. 기본적으론 장애 복구를 위해 존재하지만 프라이머리 샤드와 동일한 데이터를 가지고 있기 때문에 평상시에는 읽기 분산에도 활용된다. 

엘라스틱서치를 분산환경으로 구성하면 인덱스 생성시 기본적으로 5개의 프라이머리 샤드와 1개의 레플리카 샤드를 기본 세트로 생성한다. 각 샤드 수는 인덱스를 생성할때 옵션으로 변경할 수 있다. 

### Replica(레플리카)

(프라이머리) 샤드의 복제본을 레플리카 (Replica)라고 한다. 엘라스틱서치에서는 인덱스를 생성할때 기본적으로 1개의 복사본 세트를 생성한다. (1개의 레플리카를 만든다는게 아니다. 생성할 프라이머리 샤드의 수 * 복 ) 검색 시 레플리카가 적극적으로 활용되기 때문에 이를 이용하면 읽기 분산 성능이 좋아진다.

수평적으로 분산된 시스템을 운영할땐 언제든 장애가 나타날 수 있다. 네트워크의 특성상 특정 노드가 오프라인으로 변경될 경우, 페일오버(Failover) 메커니즘을 적극적으로 활용하는 것이 좋은데 엘라스틱서치에서는 바로 레플리카를 이용한 페일오버 메커니즘을 제공하고 있고 이로써 안정적인 클러스터 운영을 보장한다. 

인덱스를 처음 생성할때 생성할 샤드개수와 레플리카 복사본 세트 개수를 자유롭게 설정할 수 있다. 하지만 **인덱스가 생성된 이후에는 프라이머리 샤드 개수를 변경하는 것이 불가능하다.** 반면 레플리카 복사본 개수는 인덱스를 생성한 이후에도 자유롭게 변경하는 것이 가능하다. 그렇게 때문에 레플리카를 이용하면 운영 중 트래픽 증가에 대한 유연한 대응이 가능해진다. 

한 임의의 인덱스를 생성할때 만약 5개의 샤드와 1개의 레플리카 세트를 만든다고 했고 시간이 흘러 인덱스에 총 1억건의 문서가 색인 됐다고 가정해보자. 5개의 프라이머리 샤드를 설정했기 때문에 각 샤드는 2천건의 데이터를 가져가게 될것이다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTMxODMxMjc4MywtMjA2OTY4MDQ1MV19
-->