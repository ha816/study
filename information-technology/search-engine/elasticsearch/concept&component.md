...
샤드에는 두 종류가 존재한다. 첫번째는 실질적인 CRUD 서비스를 제공하는 프라이머리 샤드로 엘라스틱서치에서 핵심요소이다. 프라이머리 샤드를 일반적으로 샤드로 부른다. 두번째는 레플리카 샤드다. 기본적으로 장애 복구를 위해 존재한다. 하지만 프라이머리 샤드와 동일한 데이터를 가지고 있기 때문에 평상시에는 읽기 분산에도 활용된다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTY1ODEyNTEyMSwtMjA2OTY4MDQ1MV19
-->