...
(프라이머리) 샤드의 복제본을 레플리카(Replica)라고 한다. 엘라스틱서치에서는 인덱스를 생성할때 기본적으로 1개의 복사본 세트를 생성한다. (1개의 레플리카를 만든다는게 아니다. 실제 생성되는 레플리카의 개수 = 생성할 프라이머리 샤드의 수 * 복사본 세트 개수 ) 
...
인덱스를 처음 생성할때 생성할 샤드개수와 레플리카 복사본 세트 개수를 자유롭게 설정할 수 있다. 하지만 **인덱스가 생성된 이후에는 프라이머리 샤드 개수를 변경하는 것이 불가능하다.** 반면 **레플리카 복사본 세트 개수는 인덱스를 생성한 이후에도 자유롭게 변경하는 것이 가능하다.** 그렇게 때문에 레플리카를 이용하면 운영 중 트래픽 증가에 대한 유연한 대응이 가능해진다. 

>엘라스틱서치의 장애복구
>엘라스틱서치에서는 샤드나 노드에 장애가 발생할 경우, 즉각적인 복구가 가능하기 때문에 안정적인 클러스터 운영이 가능해진다. 페일오버 메커니즘을 구현하기 위해 레플리카는 기본적으로 원형의 프라이머리 샤드가 존재하지 노드에 생성된다. 검색 시에는 샤드와 레플리카에서 병렬로 검색을 수행할 수 있기 때문에 검색 성능이 좋아지는 이점이 있다. 
