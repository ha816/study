...
인덱스를 처음 생성할때 생성할 샤드개수와 레플리카 복사본 세트 개수를 자유롭게 설정할 수 있다. 하지만 **인덱스가 생성된 이후에는 프라이머리 샤드 개수를 변경하는 것이 불가능하다.** 반면 레플리카 복사본 개수는 인덱스를 생성한 이후에도 자유롭게 변경하는 것이 가능하다. 그렇게 때문에 운영 중 트래픽 증가에 대한 유연한 대응이 가능해진다. 이는 검색 엔진의 특성상 읽기 연산이 대부분이기 때문에 매우 유용한 기능이다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMzM5MDc0NjU0LC0yMDY5NjgwNDUxXX0=
-->