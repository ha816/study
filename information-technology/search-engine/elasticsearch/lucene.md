...
정리하자면 **루씬의 Flush** 작업은 데이터의 변경 사항을 일단 버퍼에 모아두었다가 일정 주기로 새그먼트를 생성하고 상대적으로 적은 비용의 write 함수로 디스크에 동기화를 하는 일련의 과정을 말한다.

일단 Flush 처리에 의해 세그먼트가 생성되면 커널 시스템 캐시에서 세그먼트가 캐시되어 읽기가 가능해진다. 커널 시스템 캐시가 생성되면 루씬의 ReOpen함수를 활용하여 IndexSearcher에서도 해당 캐시를 읽을 수 있는 상태가 된다. 사실 ReOpen 함수는 deprecated 되었으니 openIfChanged 함수를 대신 사용하도록 하자. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTQ1MzgwNDkzLC01NzE5NzkzODUsMTIwOT
