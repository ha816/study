...
fsync()
: 저수준의 파일 입출력 함수다. 내부 시스템 캐시의 데이터와 물리적인 디스크 데이터를 동기화하기 위한 목적으로 사용된다. 실제 물리적인 디스크로 쓰는 작업을 수행하기 때문에 운영체제 입장에서 상대적으로 많은 리소스가 사용한다.

write()
: 일반적으로 파일을 저장할 때 사용하는 함수다. 운영체제 내부 커널에는 시스템 캐시가 존재하는데 write함수를 이용하면 일단 시스템 캐시에만 기록된다. 이후에 특정 주기로 물리디스크와 동기화한다. 물리적인 디스크 쓰기 작업을 하지 않기 때문에 빠르지만 시스템이 비정상 종료될 경우에는 데이터 유실이 발생할 수 있다.

정리하자면 **루씬 Flush** 작업은 데이터의 변경 사항을 일단 버퍼에 모아두었다가 일정 주기로 새그먼트를 생성하고 상대적으로 적은 비용의 write 함수로 디스크에 동기화를 하는 일련의 과정을 말한다.
...
>**루씬 Commit**
>커널 시스템 캐시과 물리적 디스크간에 데이터를 동기화하는 작업
>실제 물리적인 디스크에 기록되기 때문에 많은 리소스가 필요하다.

>**루씬 Merge** 
>물리 디스크의 존재하는 다수의 세그먼트를 하나로 통합하는 작업
...
엘라스틱서치는 대용량 데이터를 색인하고 근실시간 검색을 할 수 있게 설계되었다. 엘라스틱서치는 각 샤드가 가지고 있는 루씬을 제어할 수 있으며, 주기적으로 각 루씬의 인메모리 버퍼에로 루씬 Flush 작업을 수행한다. 엘라스틱서치에서 **Refresh**는 모든 루씬으로 하여금 Flush를 수행시키는 작업이다. 한 클러스터에 존재하는 모든 샤드의 인덱스에서는 기본적으로 1초마다 한번씩 Refresh작업이 수행된다.
...
**엘라스틱서치에서 Flush는 루씬의 Commit을 수행하고 장애 복구를 위한 Translog를 정리하는 일련의 과정을 통칭한다.**

엘라스틱서치에서 **Flush**는 **각 루씬이 Commit 작업을 수행하고 Translog를 저장을 시작한다는 의미다.** **Translog는 샤드 장애 복구를 위해 사용하는 특수한 파일로 루씬에서는 존재하지 않는 개념이다.** 샤드는 자신에게 발생하는 모든 변경사항을 항상 Translog에 먼저 기록한 후 내부에 루씬을 호출한다. 따라서 시간이 흐를수록 Translog의 파일 크기는 늘어난다.

루씬의 Commit이 정상적으로 수행되면 Translog에 적재되었던 변경사항이 물리적 디스크에 기록되고 Commit이 정상적으로 일어난 시점 전까지 내역이 Translog 파일에서 삭제된다. 정리하자면  기본적으로 5초에 하번 Flush 작업이 수행되며, Refresh와 마찬가지로 API로 Flush 주기를 조절할 수 있지만 임의 조정은 권장하지 않는다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTExNzUyMjc2MzAsLTQyOTc1NDczNCwtOD
IyMjY3MzMxLC03MTI1MjAxMDYsMTI3NTU0ODkyNywxNDE5Mjkw
NDk1LDU3MzI3MzkyLC0xMTA4NDIyMTA3LC05MDI1MjIyNDIsLT
YyNjI5NzYsNTY3MTU4MTg3LDE2Mzc4MTk1NzcsMTk2NjAxNDkx
MiwtMTE1NDIyNDQ5NCwyMDI0NzExOTgyLC01NzE5NzkzODUsMT
IwOTIzMTIxNSwtMTM4ODk3MDY1NiwxODMwNTE1MTIyLDgxNjIx
MTA3OF19
-->