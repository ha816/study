...
(프라이머리;원) 샤드의 복제본을 레플리카 (Replica)라고 한다. 엘라스틱 서치에서는 인덱스를 생성할때 기본적으로 1개의 레플리카 세트를 생성한다. 검색 시 레플리카가 적극적으로 활용되기 때문에 이를 이용하면 읽기 분산 성능이 좋아진다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTM2MDk5ODAyMywxMTQxODU1MSwxODUzMD
