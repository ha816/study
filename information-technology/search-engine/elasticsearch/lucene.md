...
**Flush는 각 루씬이 Commit을 수행하고 장애 복구를 위한 Translog를 정리하는 일련의 과정을 통칭한다.** **Translog는 샤드 장애 복구를 위해 사용하는 특수한 파일로 루씬에서는 존재하지 않는 개념이다.** 샤드는 자신에게 발생하는 모든 변경사항을 항상 Translog에 먼저 기록한 후 내부에 루씬을 호출한다. 따라서 시간이 흐를수록 Translog의 파일 크기는 늘어난다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEyMDAwNDE0NCwtMTE3NTIyNzYzMCwtND
