...
하지만 세상에는 언제나 예외가 있다.
jvm.option이라는 파일에는 기본적인 JVM 힙 크기가 1GB로 설정되어 있는데, 실제 운영환경에서는 이를 반드시 더 큰 크기로 만들어야 한다. 힙 크기가 1GB인 이유는 엘라스틱서치를 실행할 수 있는 최소한의 힙의 크기가 1GB이기 때문이다. 이는 테스트 용도이므로 실제 운영환경에서는 힙크기는 반드시 이것보다 커야 한다. 서비스 운영을 위해 힙 크기를 변경하는 것외에는 기본 설정된 JVM 옵션을 그대로 사용하는것이 좋다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTM1MDY5MDEzOSwtMTM5Mzc5NDU5NCwtMT
