...
여기서 주목할 점은 constant pool인데 한 클래스 파일의 모든 심블릭 참조는 이 constant pool에 모두 저장한다. 따라서 각 클래스 파일은 하나의 컨스탄트 풀을 가지고 내부 버전의 컨스탄트 풀을 runtime constant pool이라 한다. 런타임 컨스탄트 풀은 클래스 파일에 컨스탄트 풀에 대응한다. 그러므로 한 타입이 로드 된 후, 그 타입으로 부터 모든 심볼릭 참조는 그 타입의
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzNTYyMDYwMTEsMTA4NjM2MDUwMCwxNj
