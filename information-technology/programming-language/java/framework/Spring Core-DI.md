...
```
위와 같은 문제를 해결하는 가장 좋은 방법은 주입 받는게 아니라 필요할때 마다 DI 컨테이너에서 빈을 찾아오면 된다. 이때 DI 컨테이너와 관련된 코드를 남가지 않는 방법이 바로 Look Up injection이다. DI 컨테이너가 빈을 Lookup하는 메서드
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjA4MzU2NjM1MSwxMTUxMTE5MzQwLDc5OD
