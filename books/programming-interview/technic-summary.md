...
>Java에서 abstract 클래스와 interface는 모두 그 자체로는 객체가 될 수 없다. abstract 클래스는 실제 구현된 메서드와 구현되지 않은 추상 메서드를 모두 가질 수 있다. 객체화할 하위 클래스에서 abstract 클래스를 상속받으면 해당 메서드를 반드시 구현해야 한다. interface의 경우도, 상속 받은 하위 클래스에서도 interface에서 정의한 메서드를 구현해야 한다. 
...
>**Collection 인터페이스?**
>Java에서 Collection 인터페이스를 상속하는 클래스는 List, Set이 대표적이다. 선형 구조의
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE5NzQzNTA1NTcsLTQxMzA5MDA5MiwtMT
