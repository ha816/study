...
> **Docker는?**
> 도커는 **컨테이너 기반의 오픈소스 가상화 플랫폼**이다. 가벼운 컨테이너를 만들고, 탑재하고 실행할 수 있는 플랫폼이다. 다르게 이야기하면 **컨테이너 기반의 오픈소스 가상화 플랫폼**이다. 다양한 프로그램, 실행환경을 컨테이너로 가상화 하는데 컨테이너는 격리된 공간에서 프로세스가 동작하는 기술입니다. 기존의 가상화 방식은 주로  **OS를 가상화** 했는데 OS이기 때문에 성능이 느렸고 도커는 프로세스를 가상화하기 때문에 빠르게 동작한다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNTI2NjIzNjY1LC0yMjM1NjU2NTgsNDY2MD
