...
>Sharding은 수평 파티셔닝을 일컫는 말이다. Sharding을 하려면 Sharding key를 정하고 이를 기준으로 테이블을 나누어야 한다. dl파티션 테이블에서 인덱스는 전부 로컬 인덱스에 해당한다. 모든 인덱스는 파티션 단위로 생성되며, 파티션에 관계없이 테이블 전체 단위로 글로벌하게 하나의 통합된 인덱스는 지원하지 않는다
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTkwNjcwMjc3MSwtNTcwMDE3MTU4LDEyOT
