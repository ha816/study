...
>두 컨텍스트의 차이는 애플리케이션 컨텍스트의 생성시기가 다른 점을 이야기 할 수 있다. 루트 웹 애플리케이션 컨텍스트는 스프링 동작시 생성된다. 서블릿 웹 애플리케이션은 지연 생성으로 실제 요청이 들어올때 생성된다. 참고로 루트 웹애플리케이션 컨텍스트에는 모든 서블릿 애플리케이션에서 사용하면 좋을 빈들이 올라간다. 대표적으로는 DataSource가 있다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwMDEzNzI5NTUsMzgyMTQwODk3LC0xNz
