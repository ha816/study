...
>루트 웹 애플리케이션 컨텍스트는 웹 애플리케이션 단위로 하나씩 생성되는 DI 또는 IoC 컨테이너이다. 한 웹 애플리케이션에서 다수의 서블릿을 가질 수 있으므로, 각 서블릿마다 서블릿 웹 애플리케이션 컨텍스트가 생성되고 자신의 빈을 관리한다.
>루트 웹 애플리케이션 컨텍스트는 서블릿 웹 애플리케이션 컨텍스트의 부모가 되며 이들은 서블릿 웹 애플리케이션 컨텍스트에서 공통으로 사용될 빈(DataSource, Transaction 등)을 생성하고 관리한다.
>둘의 차이로는 애플리케이션 컨텍스트의 생성시기가 다른 점을 이야기 할 수 있다. 루트 웹 애플리케이션 컨텍스트는 스프링 동작시 생성된다. 서블릿 웹 애플리케이션은 지연 생성으로 실제 요청이 들어올때 생성된다. 참고로 루트 웹애플리케이션 컨텍스트에는 모든 서블릿 애플리케이션에서 사용하면 좋을 빈들이 올라간다. 대표적으로는 DataSource가 있다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMzgyMTQwODk3LC0xNzEyNTU3NzA2LDE5OD
UzNTYwMzQsMTA5MjE0NjM3MiwtMTkzODE0MjkwMiwtMjAzOTQ4
MjI4OCwtMTE3OTk1ODU2OCwtMTk2NDg0MTI4MCwtMTgwMDczNz
k3MywxNDA1NDU0NTQ3LC00MDg4OTQxOTQsLTEwMTEyODgzNzks
LTIwNTc4NDk1MzIsOTE2NTcyNzM5LC0yNTk0NTQyMTIsLTIwOD
YwNDcwMDgsMTQxMDQ3MTE0MywtMTI0MDQ0NTc0NywtMzEyNzQz
NzMxLC01MzAwMDE3NTRdfQ==
-->