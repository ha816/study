...
>PreparedStatement는 Statement와는 다르게 한번 실행된 쿼리의 일부 작업을 저장해두어, 매번 같은 작업 과정을 거치지 않고 건너뛴다는 것이다. PreparedStatement에서는 쿼리에 바인딩 변수를 사용할 수 있다. 덕분에 Statement 쿼리에 비해서 서버에서 보관해야하는 정보가 줄어 메모리 사용량을 줄일 수 있다. 또한 변수에 SQL Injection이 들어오는 것을 방지할 수 있다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNDM3MDE1ODQ5LDE3ODI1MjMyMjksMTAwND
