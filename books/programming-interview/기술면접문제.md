...
> HTTP2는 성능과 속도 모두 잡기 위해 HTTP1.1에서 지원하지 않던 새로운 주요 기능 4가지를 제공한다. 요청과 응답시 멀티플렉싱을 통하여 레이턴시를 줄이고(MultiPlexing), HTTP header필드를 압축하여 프로토콜 오버헤드를 줄이며(Header Compression), 요청 우선순위(Prioritization)와 서버푸시기능(Server Push)
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwMTEwNTM5NzcsLTU1MzU0MTI3NiwtMT
