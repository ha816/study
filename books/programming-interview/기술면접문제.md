...
>**클러스터와 리플레케이션의 차이**
>Cluster는 Network DataBase 스토리지를 사용하며, 모든 데이터베이스를 실시간으로 동기화하고 확인한다. 
>Replication도 마스터 데이터베이스를 슬레이브 데이터베이스에 동기화 한다. 하지만 replication은 동기화가 비동기적으로 발생한다. 따라서 어떤 데이터베이스에는 데이터가 업데이트되어 있지만, 다른 데이터베이스에서는 업데이트되지 않을 수도 있다.  또한, 마스터와 슬레이브로 나누어지기 때문에 데이터를 변경하는 쿼리는 마스터 데이터베이스에만 요청할 수 있다. 
>Cluster는 모든 데이터를 실시간으로 동기화하고 확인한다. 
>Replication은 cluster에 비해 값의 변경이 매우 빠르기 때문에 주로 실시간 동기화가 필요 없는 경우 Replication을 사용한다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjAwODYwMzQxNSwxNDU0OTU1MzEwLDE4Mj
kyMzY1MDYsLTIwOTUxMDA0MzcsLTE2NzYzMzE2OTAsMTc4MjUy
MzIyOSwxMDA0NTE5MTI4LDEwNjY3NTUwODUsLTE4NDAyNTEyOD
IsNTcyMTY4OTAwLDE1MzY1MTg3OSwtMzA1ODkxNTkzLDE4MjQy
MTA0OTYsLTEzNDI3ODU1NywtMTM0NTE3MTIzMiwxODEzOTQxOD
AxLDk5OTU1Mjk0LDE2NjAwMzM4MTAsLTU1MzU0MTI3NiwtMTY0
NDM3OTQwOF19
-->