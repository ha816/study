...
>Statement는 사용하는 쿼리문 자체를 DB서버에 보낸다. 때문에 SQL Injection에 취약하며 사용하는 쿼리가 비슷하더라도 성능 이점을 얻을 수 없다. PreparedStatement 쿼리에는 바인딩 변수를 사용할 수 있다. 템플릿화 된 쿼리를 DB서버에 미리 알려주어 일부 작업의 결과를 저장한다. 해당 쿼리를 반복적으로 요청시 저장해둔 작업의 결과를 재사용하여 쿼리 수행시 성능상 이득이 있다. 또한 SQL Injection의 예방할 수 있다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwOTUxMDA0MzcsLTE2NzYzMzE2OTAsMT
