...
>UDP는 발신지와 수신지간의 논리적인 경로가 없다. 각 패킷은 서로 독립적으로 다른 경로로 전송된다. UDP는 전송순서를 지키지 않고, 손실이 있을수 있기 때문에 신뢰성있는 데이터의 전송을 보장하지는 못합니다. 대신 TCP보다 속도가 빠르며 네트워크 부하가 적다는 장점이 있다.
...
> Multiplexing: 요청과 응답시 멀티플렉싱을 통하여 레이턴시를 줄인다.
> Prioritization: 요청 우선순위 설정이 가능.
>Header Compression: HTTP header필드를 압축하여 프로토콜 오버헤드를 줄인다.
>Server Push: 필요한 리소스를 클라이언트 요청 없이 서버에서 보내준다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTQ5MDcyMzM1MSwtNTUzNTQxMjc2LC0xNj
