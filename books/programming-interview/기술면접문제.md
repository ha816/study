...
>**클러스터와 리플레케이션의 차이**
>MySQL replication은 마스터 데이터베이스를 슬레이브 데이터베이스에  언뜻 보면 MySQL cluster와 비슷하지만, 말 그대로 분산환경을 만들어서 [single point of failure](http://en.wikipedia.org/wiki/Single_point_of_failure)를 없애려는 cluster와는 달리 MySQL replication은 단순히 데이터를 복제한다.  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTkyMDI4MTg4MSwxODI5MjM2NTA2LC0yMD
