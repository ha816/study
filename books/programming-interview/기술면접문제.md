...
>Statement는 사용하는 쿼리문 자체를 DB서버에 보낸다. 때문에 SQL Injection에 취약하며 사용하는 쿼리가 비슷하더라도 성능 이점을 얻을 수 없다. PreparedStatement 쿼리에는 바인딩 변수를 사용할 수 있다. 템플릿화 된 쿼리를 DB서버에 미리 알려주어 일부 작업의 결과를 저장한다. 파싱된 쿼리를 요청마다 재사용하여 반복적인 쿼리 수행시 성능상 이득이 있다. 또한 SQL Injection의 예방할 수 있다. 
>PreparedStatement는 Statement와는 다르게 한번 실행된 쿼리의 일부작업을 저장해두어 , 다음 요청 쿼리부터는 일부 과정을 거치지 않고 저장된 결과를 재사용한다. 
>또한, PreparedStatement에서는 쿼리에 바인딩 변수를 사용할 수 있다. 덕분에 Statement 쿼리에 비해 서버에서 보관해야하는 정보가 줄어 메모리 사용량을 줄일 수 있다. 또한 변수에 SQL Injection이 들어오는 것을 방지할 수 있다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTYwNDk4ODQ3LC0xNjc2MzMxNjkwLDE3OD
I1MjMyMjksMTAwNDUxOTEyOCwxMDY2NzU1MDg1LC0xODQwMjUx
MjgyLDU3MjE2ODkwMCwxNTM2NTE4NzksLTMwNTg5MTU5MywxOD
I0MjEwNDk2LC0xMzQyNzg1NTcsLTEzNDUxNzEyMzIsMTgxMzk0
MTgwMSw5OTk1NTI5NCwxNjYwMDMzODEwLC01NTM1NDEyNzYsLT
E2NDQzNzk0MDgsMTQ1MjMzNzY5MCw3MzY5NjMwMTIsMjA1NzY4
ODcxN119
-->