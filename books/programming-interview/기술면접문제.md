...
> HTTP2는 성능과 속도 모두 잡기 위해 새로운 기능 4가지를 제공한다. 
> 요청과 응답시 멀티플렉싱을 통하여 레이턴시를 줄인다.(Multiplexing), 요청 우선순위 설정(Prioritization)이 가능하고 HTTP header필드를 압축하여 프로토콜 오버헤드를 줄인다.(Header Compression), 요청 없이 필요한 리소스를 서버에서 보내주는 서버푸시기능(Server Push)
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE1MTk5OTU1NTIsLTU1MzU0MTI3NiwtMT
