...
>PreparedStatement는 Statement와는 다르게 한번 실행된 쿼리를  매번 쿼리 분석 과정을 거치지 않고 재사용한다는 점이다. 
>PreparedStatement를 사용하면 쿼리 분석이나 최적화의 일부작업을 처음 한번만 수행해 별도로 저장해 두고, 다음 부터 요청되는 쿼리는 저장된 분석결과를 재사용한다. 이렇게 하여 매번 쿼리를 실행해야 하는 쿼리분석이나 최적화의 일부 작업을 건너띌 수 있다. PreparedStatement에서 "?" 바인딩 변수 또는 변수 홀더(variable holder)라고 하는데, 바인딩 변수를 사용하면 쿼리를 템플릿화 할 수 있고, Statement 쿼리에 비해서 쿼리 문장의 수를 대폭 줄일 수 있다. 덕분에 MySQL 서버에서 보관해야하는 정보가 줄어 메모리 사용량을 줄일 수 있다.
결론적으로 PreparedStatement의 성능적 장점은 한번 실행된 쿼리는 매번 쿼리 분석 과정을 거치지 않고 재사용한다는 점이다. SQL 문장의 길이가 길다면 SQL 문장 자체가 네트워크로 전송되지 않고 바인딩할 변수 값만 서버로 전달하기 때문에 네트워크 측면에서 다소 효율적이다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwMTE4MjczNDEsMTA2Njc1NTA4NSwtMT
g0MDI1MTI4Miw1NzIxNjg5MDAsMTUzNjUxODc5LC0zMDU4OTE1
OTMsMTgyNDIxMDQ5NiwtMTM0Mjc4NTU3LC0xMzQ1MTcxMjMyLD
E4MTM5NDE4MDEsOTk5NTUyOTQsMTY2MDAzMzgxMCwtNTUzNTQx
Mjc2LC0xNjQ0Mzc5NDA4LDE0NTIzMzc2OTAsNzM2OTYzMDEyLD
IwNTc2ODg3MTcsLTY1MzA1NTI5OCwtMTQyOTA5NDI2MywtMjcx
Njg2MjM5XX0=
-->