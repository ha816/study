...
package
: test 상태가 성공적으로 실행된 다음에 실행되며 WAR나 JAR파일 같은 결과물을 생성한다. 파일은 target 디렉터리 루트에 저장되는데, 독립적으로 실행가능한 파일이 아니다. 왜냐하면 기본적으로 모든 의존 라이브러리를 내장하지 않고 있고 의존성은 여전히 클래스 패스에 포함되어 있어야 하기 때문이다. pom.xml의 packaging 태그로 결과물 패키지 타입을 정할 수 있다. jar:jar , war로 설정되면 war:war 골이 실행된다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwNjc0NzY2OTksMTAzMzI2MDA0MywxMT
