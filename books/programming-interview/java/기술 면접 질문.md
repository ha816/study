...
> 32bit 컴퓨터는 CPU 안에 Register의 처리 단위가 32bit임을 말한다. 이 레지스터가 표현할 수 있는 값의 범위가 CPU가 한번에 처리할 수 있는 주소값의 범위가 된다. 그래서 32bit 컴퓨터는 메모리 주소를 4GB까지밖에 인식하지 못한다. $2^{32} =4 * 2^{10} *2^{10} *2^{10}$
> 따라서 64bit 컴퓨터는 한 클럭에서 한번에 두개의 integer(32bit) 정수값을 처리할 수 있다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTc5MTE5NTA3MywtMTY2MTkwMzc2OCwxNj
