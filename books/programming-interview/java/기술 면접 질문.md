...
>데드락 처리는?
>데드락 처리에는 예방(Prevention), 회피(Avoidance) 그리고 탐지 및 회복(Detection & Recover) 방법이 있다.
...
> WAS와 Web Server의 차이
> WAS(WebApplication Server)의 약자로 동적인 웹 서비스를 제공하기 위해 사용한다. 일반적으로 WAS는 WebServer와 WebContainer(Servelet Container)로 구성된다. 
> Web Server는 정적인 웹 서비스를 제공하며, WAS에 비해 기능이 가볍다. 아파치 서버, Nginx가 대표적이다. 

> WebContainer(Servelet Container
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTMxNTk5NDQxOSwtMTA4MzgwNTg1MywtMT
Y2MTkwMzc2OCwxNjU2OTk4NjYyLDE3NzI3Njk4ODIsLTU5Mzg3
MTc3MF19
-->