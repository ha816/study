...
내부적으로 배열을 사용하는 리스트이다. 즉 배열의 특징을 그대로 가져간다.(랜덤 접근, 전체 복사) 배열의 시작 위치나 중간 위치에 새로운 원소를 추가하려고 하면, 그 뒤에 있는 원소들은 모두 이동해야 한다. 많은 원소를 삭제해도 차지하는 배열의 크기는 줄어들지 않는다. 정리하자면 원소 개수가 계속 변경되는 리스트라면 ArrayList의 특징이 단점으로 작용할 수 있다. 

## LinkedList

원소들을 배열에 저장하지 않고, 리스트 안에서 다음 원소를 가리키는 내부 참조를 이용하는 리스트 이다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjAwMzU5ODkyOSwtMjAyMjEzMjk3MywtMT
