...
스플릿 방식은 스레드 별로 처리해야할 스텝을 나누어 작업하는 방식이다. 또한 하나의 스레드에는 여러 스텝들을 연속으로 실행하도록 만들 수 있다.  병렬 처리할 스텝을 세부적으로 지정할 수 있는 장점이 있는 방면 특정 스레드에서 처리해야할 개수가 많아지면 스레드 갯수를 조정하거나 비정상적으로 종료될때 처리가 어려워지는 단점도 있다. 
...
<batch:job id="refundCashbackJob" job-repository="jobRepository">  
 <batch:split id="refundCashbackSplit" task-executor="taskExecutor">  
 <batch:next on="*" to="refundCashbackNotificationStep" />  
 <batch:flow>  
 <batch:step id="nonAlwaysSaleRefundCashbackStep">  
 <batch:tasklet transaction-manager="transactionManager">  
 <batch:chunk reader="nonAlwaysSaleRefundCashbackReader" writer="nonAlwaysSaleRefundCashbackWriter" commit-interval="1" skip-limit="99999999" retry-limit="0">  
 <batch:skippable-exception-classes>  
 <batch:include class="com.tmoncorp.delivery.exception.SkippableException"/>  
 </batch:skippable-exception-classes>  
 </batch:chunk>  
 <batch:listeners>  
 <batch:listener ref="longQueryEnableListener"/>  
 </batch:listeners>  
 </batch:tasklet>  
 </batch:step>  
 </batch:flow>  
 <batch:flow>  
 <batch:step id="alwaysSaleRefundCashbackStep">  
 <batch:tasklet transaction-manager="transactionManager" >  
 <batch:chunk reader="alwaysSaleRefundCashbackReader" writer="alwaysSaleRefundCashbackWriter" commit-interval="1" skip-limit="99999999" retry-limit="0">  
 <batch:skippable-exception-classes>  
 <batch:include class="com.tmoncorp.delivery.exception.SkippableException"/>  
 </batch:skippable-exception-classes>  
 </batch:chunk>  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTc3ODM2NDM0LC0xMzA0NjMzNDE5LC0yNz
E1ODgwNTEsLTYyODQ2OTcyMywtMTgyMTAyNDQ1OCwtMTI5ODg3
Mjk0MiwxNTU4ODQ5Njg4LC0xNDA3MDExMzk2LDkyMjUwOTMyOC
wxNzQ2ODQ1MzUyLC0yMTEyODgwOTg0LC0xMTExNjYxOTE0LDk5
NTA4NDE2MSwxODQxNDc3MTQsLTc5OTc3MTA1NCwtMTQ1MzU2Mz
cxNSwtODE5MzkyMjQzLDE2OTQwMjU2NjksNzAwOTIzNjMzLDE2
MDgyNjk0XX0=
-->