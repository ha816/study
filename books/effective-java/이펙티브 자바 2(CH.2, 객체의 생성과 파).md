...
정적 팩터리 메서드는 반환 타입의 하위 타입이기만 하면 어떤 클래스의 객체를 반환하든 상관이 없다. 클라이언트는 팩터리가 던져주는 객체가 어느 클래스의 인스턴스인지 알  수 없고 알 필요도 없다. 그저 반환 타입이 인스턴스를 사용하기에 문제만 없으면 된다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzMDM1Njk3MzIsLTM4MTI4NjgyNywtOT
