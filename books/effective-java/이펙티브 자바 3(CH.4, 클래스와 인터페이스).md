...
### 14. public 클래스 안에는 public 필드를 두지 말고 접근자 메서드를 사용해라!
 ...
 private 필드와 public 접근자 메서드(getter, setter)를 사용하자. 패키지 바깥에서 접근할때 사용하려는 클래스의 접근자를 사용하여도록 하자. (예를 들면, getter) 그러면 사용되는 클래스 내부 표현 방식을 언제든 바꿀 수 있게 된다. 반면에 public 클래스의 데이터 필드를 공게하게 되면, 이를 직접 사용해서 가공하는 다수의 클라이언트가 생겨날 것이므로, 내부 표현 방식을 ㄱ
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTg4NjU2ODA3MywxOTc2OTk4MzAzLC0xNj
