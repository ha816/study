...
```
defer는 지연하다, 연기하다라는 뜻입니다. defer는 구독하는 순간까지 행동을 지연하여 시퀀스를 생성하는 메서드로 결과적으로 서로 다른 구독자에 대해 다른 데이터를 생성할 수 있습니다. 
...
subscribe(Consumber<T> dataConsumer, Consumer<Throwable> errorConsumer); ()
subscribe(Consumber<T> dataConsumer, Consumer<Throwable> errorConsumer, Runnalbe completeConsumer);
subscribe(Consumber<T> dataConsumer, Consumer<Throwable> errorConsumer, Runnalbe completeConsumer, Consumer<Subscription> subscriptionConsumer);
subscribe(Subscriber<T> subsriber)
```
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwOTMyMzExNDAsLTEyMjA2ODE1ODgsLT
Q0Nzc4ODEyMiwxNDg0Njk4NjE4LC0xNzk4MzQyNzQ1LDE2NTk2
MzE1OTcsNzYzNjMyNzEsNjA1NDMwNDgyLDgwOTM3NDg5MCwtNz
k1MDA4MDMyLDUxMzM0ODY2NCwtMTAwNDExODgyNiwyMDc4MDk3
NzQxLC0xNTk5NDE1NjM3LDY2NTU4OTUyNiwxMjQwNDI1NDY2LC
0zODYwMzYyOSwtMTMzOTIxODM3NywtMTE2MDY5OTU3Nyw4ODk2
NjQxOTRdfQ==
-->