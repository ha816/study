...
문서 수정을 위해서 매번 새로운 세그먼트를 생성하면, 점점 시간이 지나면서 작은 크기의 세그먼트가 늘어나게 된다. 이는 사용자가 문서를 검색할때 마다 많은 수의 세그먼트들을 검색하게 되어 검색 성능이 떨어지게 된다. 심지어 불용 처리한 데이터들도 사실 세그먼트에 남아있기 때문에 결과적으로 세그먼트의 전체 크기도 점점 커지게 된다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTEzNDcxNjg0NywxMTM1MTk1Mjk5LDIxMD
