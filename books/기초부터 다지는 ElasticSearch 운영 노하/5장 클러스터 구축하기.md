...
페이지 캐시는 애플리케이션들이 사용하지 않는 미사용 메모리를 활용해서 동작하기 때문에 페이지 캐시를 최대한 활용하기 위해서는 애플리케이션이 사용하는 메모리를 줄이는 것이 좋다. 특히 ES와 같이 I/O 작업이 빈번한 경우 가급적 많은 메모리를 페이지 캐시로 제공하여 작업이 메모리 내에서 끝나도록 하는 것이 성능에 큰 도움이 된다. 

이러한 이유로 공식 문서에는 물리 메모리의 절반 정도를 힙 메모리로 할당할 것을 권장한다. 굳이 많은 메모리가 필요하지 않다면 절반이하로 설정해도 괜찮다. 

# 마치며 

이번 장에서 살펴본 주요 내용을 정리하면 아래와 같다. 

1. 마스터 노드는 클러스터 전체 메타 데이터를 관리하며, 클러스터 내에선 반드시 한대 이상이 존재해야 한다. 클러스터의 안정성을 위해서 minimum_master_nodes를 과반수로 설정할 수 있도록 홀수 값으로 구성하는 것이 좋다. 그래야만 Split Brain 현상을 방지할 수 있다. 
2.색인, 검색의 성능을 높이기 위해 노드 증설이 필요한 경우 데이터 노드만 증설하면 되는데, 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTU2MDAwMjk4OSwtMTc5Nzg3MTMwMSwtMT
A1NTkzMDg2Nyw1OTM0ODYxMzYsMTM2NzUzNDA1NCwxNDcxNzM2
NDAsLTE2NjU5NTMxNTksMzA3NDczMTUsMzA1NDMyNzE0LC0yOD
