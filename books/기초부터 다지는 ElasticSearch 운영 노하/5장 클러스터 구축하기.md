...
bind_host는 외부에서 들어오는 요청을 듣기 위해 할당되는 ip주소이다. 반면에 publish_host는 클러스터 내부에서 ES 노드간 통신으로 사용되는 공통의 ip주소이다. 따라서 publish_host는 단일 인터페이스를 가지는데, 클러스터 내에서 
The publish host is the single interface that the node advertises to other nodes in the cluster, so that those nodes can connect to it. Currently an Elasticsearch node may be bound to multiple addresses, but only publishes one
...
이럴땐 해결방법으로 network.host를 0.0.0.0으로 설정하면 된다. 0.0.0.0으로 설정하면 ES가 bind_host로  10.10.10.10과 127.0.0.1 두 IP를 모두 사용할 수 있도록 해준다. 사실 0.0.0.0의 의미는 어떤 
...
그렇다면 network.host를 0.0.0.0으로 하면 모든 이슈가 해결될까? 그건 아니다. 클러스터 노드들이 사용하는 publish_host도 0.0.0.0이다. 즉 publish_host가 0.0.0.0이 되면 다른 ES 노드에서 내부 통신을 위한 host가 0.0.0.0이 된다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNjAwMzk4NzY1LDEwNjg2MTY0NTYsMTY5OD
A4OTEyMCw2OTUzMTc0MjIsLTEyODI5MDk0MiwtMTA4MzgzNzk5
MywtNzU2MDIwMDAzLDU5MDI4NzA1OCwxODI2NTEzODUzLC00NT
I2NjMxNjAsMTY4MDg1NTgyNCw2OTcxMzE5MTIsNTU1NjM3NDg4
LDU5ODY2MDA0MiwtMTQ1NDk4MDc0NiwtMTY3ODIwNzY3OCwxNz
