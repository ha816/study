...
이러한 해시함수를 자바에서 구현한것이 바로 Object 클래스에 정의된 hashCode 메서드이다. 이 메서드를 호출한 객체의 내부 메모리 주소를 integer값으로 반환한다. 짚고 넘어 갈것은 해시함수의 `적당한`이란 의미에 맞게 분명히 다른 객체가 다른 메모리 주소를 가져도 같은 integer값이 밚

바로 이 hashCode()가 자바에서 구현한 hash function이다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTgyNjc2NjYwNSwxNTcwNTQyMzI2LDMxOD
