...
앞서 봤던 완전탐색 방법 점화식을 동적계획법에 어울리게 메모이제이션을 쓸수 있도록 고쳐보도록 하자. 메모이제이션을 쓰기위해서는 지금까지 만든 부분 경로인 $path$가 쓰이는 곳은 두 군데로, 경로가 완성되었을때 전체 경로의 길이를 계산하는 부분과 어떤 도시를 전에 방문한적이 있는지 확인하는 부분이다. 
$$getMinCost(from, visited)$$$$= \min({getMinCost(to, visited + next) + dist[from][to]})$$

$getMinCost(from, visited)$는 시작 노드가 from이고 $visited$를 제외한 모든 노드들을 탐색하는 경로들 중 경로합이 최소인 값을 반환한다. 앞선 완전탐색 점화식과 다르게 $paths$ 대신 $visited$를 사용한다. 

이 알고리즘의 복잡도는 $2^n * n^2$이다. 왜냐하면 관찰해야하는 visited의 모든 갯수가 $O(2^n)$이고 visited에 해당하지 않는 것들 중에서 순서에 상관없이 선택하는 경우의 수는 $nC_2$이기 때문에 복잡도는 $O(2^n * n^2)$이다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTY0NzA3MzUzNiwxOTEzMjI4MDMwLC0xOD
U1NTYwMiwyMDkwODE3MDk3LC0xODExMzYzMTExLC0xNzQwOTUw
MTk3LC0yMDE4MzIyNDk1LC0xODIxMjI0NTkxLDUwNjY1ODA0My
w4OTY0MzU1MTksNTgxOTQ5ODc3LC0xNzU5NTk1NTk1LC0xODAx
NjQ3NTE5LDEzMDM3Njc2NTksLTkzNzU5ODEzMCwtNDY1MTQ0Mj
I2LDc4NTQ3MjE0LC0yMzYwMzM4MTgsMTYwNDA5OTY4MCwtMTY0
NjI4MzA1MF19
-->