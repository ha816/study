...
$getMinCost(from, visited)$는 $visited$를 제외한 모든 노드들을 탐색하는 경로들 중 시작 노드가 from이고 경로값이 최소인 값을 반환한다. 앞선 완전탐색 점화식과 다르게 $paths$ 대신 $visited$를 사용한다. 

이 알고리즘의 복잡도는 $2^n * n^2$이다. 왜냐하면 관찰해야하는 visited의 갯수가 $O(2^n)$이고 visited에 해당하지 않는 것들 중에서 순서에 상관없이 선택하는 경우의 수는 $nC_2$이기 때문에 복잡도는 $O(2^n * n^2)$이다.



> Written with [StackEdit](https://stackedit.io/).
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTc1NTcyNzU2OSwtMTg1NTU2MDIsMjA5MD
