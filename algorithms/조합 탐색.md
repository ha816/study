...
반면에 $e$가 MST에 포함되지 않으면, $T+e$은 cycle $C$를 가지게 된다. 이 cycle은 $F$에 포함되지 않은 간선을 가진다. 왜냐하면 $e$가 $F$에서는 cycle을 가지지 않지만 $T$에선 가지기 때문이다. $f$를 $C$에 있지만 $F$에는 없는 하나의 간선이라고 하자. 그러면 $f$는 $T$에도 속한다. 따라서 $f$의 가중치는 $e$보다 크거나 같다. $T-f+e$는 트리이고,  $T$에 비해서 가중치가 같거나 작다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE1OTczMzgxNzIsOTczMTE4NTQ0LC00MD
QwODIwOCw5NzUwMDk3OTQsNTczMDUyNDU3LC05MDc3MTExODUs
LTE5MzIxODUwMDUsLTIxMDg3NDE5NjksNTk2MjQzNTg0LC03Mz
c1Mzk3MDEsNjU2NDIxMjM5LDE1NTg2ODQzNTIsLTExMTY4ODc5
MSw4OTM3OTk5MjcsMTEyNTk5MDgwNywtMzY5MDU5NzQ4LDgzND
c1MjcxNiwtNTQ2ODU3NjI3LDEwMDg3OTE5NzIsMTQxMDU2ODIy
OF19
-->