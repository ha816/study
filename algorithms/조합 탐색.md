...
한정(bounding)
: 한정은 하위영역 내에서 최소 또는 최대의 범위(upper and lower bound)를 계산하는 과정을 말한다. 이 과정을 통해 단계를 절단 (pruning) 이라고 부른다.

임의의 하위영역 A에서 lowerbound가 계산되었고 이미 앞서 계산된 다른 하위영역 B 의 upper bound 보다 크다면 A 를 탐색에서 제외하는 간단한 방법이다. 
...
> Written with [StackEdit](https://stackedit.io/).
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTA3ODg1NzMyNCwtMTM4MTg2NzM2MSwtMT
