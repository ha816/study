...
모든 노드들이 연결되어 있고, 가중치가 주어진 그래프 $G$가 주어졌다고 하자. 알고리즘은 모든 간선을 하나씩 검토하고 그중에 기존에 신장트리에 간선을 추가했을때 cycle을 생성하는 경우를 제거한다. 그리고 그래프 G가 모든 노드들이 연결 될 수 있기 때문에 모든 

그러면 정의에 따라서 $Y$는 cycle이 없고 하나의 트리로만 구성이 된다. $Y$는 모든 노드를 가기 때문에 disconnected할 수 없다. 

왜냐하면, $Y$의 두 노드를 합친 가장 먼저 마주한 간선이 이미 알고리즘에서 더해졌기 때문에
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTY2OTgwNzk0NCw1NzMwNTI0NTcsLTkwNz
cxMTE4NSwtMTkzMjE4NTAwNSwtMjEwODc0MTk2OSw1OTYyNDM1
ODQsLTczNzUzOTcwMSw2NTY0MjEyMzksMTU1ODY4NDM1MiwtMT
ExNjg4NzkxLDg5Mzc5OTkyNywxMTI1OTkwODA3LC0zNjkwNTk3
NDgsODM0NzUyNzE2LC01NDY4NTc2MjcsMTAwODc5MTk3MiwxND
EwNTY4MjI4LC02NjE0NzczNTMsNjI4NDMzMjIsLTQ1MjA0NzUw
MF19
-->