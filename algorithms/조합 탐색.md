...
한정(bounding)
: 한정은 하위영역 내에서 최소 또는 최대의 범위(upper and lower bound)를 계산하는 과정을 말한다. 이 계산 결과를 이용해서 분기한정법의 핵심인 탐색공간(search space)를 줄이는 것이 가능한데 단계를 절단 (pruning) 이라고 부른다.
...
> Written with [StackEdit](https://stackedit.io/).
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEyMTg3NDczNDcsLTEzODE4NjczNjEsLT
