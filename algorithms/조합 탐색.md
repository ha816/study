...
모든 노드들이 연결되어 있고, 가중치가 주어진 그래프 $G$가 주어졌을때, $Y$를 Krukal 방법으로 만들어진 결과 최소신장트리 MST라고 하자.  그러면 정의에 따라서 $Y$는 cycle이 없고 하나의 트리로만 구성이 된다. $Y$는 모든 노드를 가기 때문에 disconnected할 수 없다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNTczMDUyNDU3LC05MDc3MTExODUsLTE5Mz
