...
하나의 그래프 $G$는  다수의 신장트리(spanning tree)를 가질 수 있고 그 중에 가중치의 총합이 최소인 트리가 우리가 찾고자 하는 최소신장트리이다. $F$를 앞선 어떤 상황에서 더해진 간선들을 말하고, 몇몇 MST는 이 $F$를 포함한다.

맨처음에는 $F$가 비어있고 


We show that the following proposition  _**P**_  is true by  [induction](https://en.wikipedia.org/wiki/Mathematical_induction "Mathematical induction"): If  _F_  is the set of edges chosen at any stage of the algorithm, then there is some minimum spanning tree that contains  _F_.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE5MTkyNjY5OTYsOTczMTE4NTQ0LC00MD
