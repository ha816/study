...
한정(bounding)
: 한정은 하위영역 내에서 최소 또는 최대의 범위(upper and lower bound)를 계산하는 과정을 말한다. 임의의 하위영역 A에서 lowerbound가 계산되었고 

에서 하위영역 A 의 lower bound 가 이미 검사된 다른 하위영역 B 의 upper bound 보다 크다면 A 를 탐색에서 제외하는 간단한 방법이다. 이러한 단계를 절단 (pruning) 이라고 부른다.

Pruning refers instead to the process of discarding subtrees in the branch decision tree rooted at subproblems whose solution is impossible or worst with regard to the feasible solution of the original problem. 
...
> Written with [StackEdit](https://stackedit.io/).
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTU1NTc3NDY1NCwtMTM4MTg2NzM2MSwtMT
M3OTQ1NzIxMCw5NTkzNjM5NTUsMTE2MDE4MTIzNiwxNDc3MjM3
MDk4XX0=
-->