...
하나의 그래프 $G$는  다수의 신장트리(spanning tree)를 가질 수 있고 그 중에 가중치의 총합이 최소인 트리가 우리가 찾고자 하는 최소신장트리이다. 그리고 그래프 $G$ 항상 MST를 가진다.

$F$를 앞선 어떤 상황에서 더해진 간선들을 말하고, 몇몇 MST는 이 $F$를 포함한다. 이렇게 $F$를 포함하는 MST를 T라고 하자. 

만약 다음에 더하기 결정된 간선 $e$ 가 만약 MST에 포함되면 $F+e$는 옳다.

반면에 $e$가 MST에 포함되지 않으면, $T+e$은 cycle $C$를 가지게 된다. 이 cycle은 $F$에 포함되지 않은 간선을 가진다. 왜냐하면 $e$가 $F$에서는 cycle을 가지지 않지만 $T$에선 가지기 때문이다. $f$를 $C$에 있지만 $F$에는 없는 하나의 간선이라고 하자. 그러면 $f$는 $T$에도 속한다. 따라서 $f$의 가중치는 $e$보다 크거나 같다. $T-f+e$는 트리이고,  $T$에 비해서 가중치가 같거나 작다. 결과적으로 $T-f+e$가 바로 MST이며 $F+e$를 가진다. 



-   Clearly  _**P**_  is true at the beginning, when  _F_  is empty: any minimum spanning tree will do, and there exists one because a weighted connected graph always has a minimum spanning tree.
-   Now assume  _**P**_  is true for some non-final edge set  _F_  and let  _T_  be a minimum spanning tree that contains  _F_.
    -   If the next chosen edge  _e_  is also in  _T_, then  _**P**_  is true for  _F_  +  _e_.
    -   Otherwise,  _e_  is not in  _T_, and  _T_  +  _e_  has a cycle  _C_. This cycle contains edges which do not belong to  _F_, since  _e_  does not form a cycle in  _F_  but does in  _T_. Let  _f_  be an edge in  _C_  but not in  _F{\displaystyle \cup \{e\}}![{\displaystyle \cup \{e\}}](https://wikimedia.org/api/rest_v1/media/math/render/svg/ae9627b70f3a07fe99d2e7e043559b56323cf37d)_. Note that  _f_  belongs also to  _T_, and by  _**P**_  has not be considered by the algorithm, and hence a weight at least as large as  _e_. Then  _T_  −  _f_  +  _e_  is a tree, and it has the same or less weight as  _T_. So  _T_  −  _f_  +  _e_  is a minimum spanning tree containing  _F_  +  _e_  and again  _**P**_  holds.
-   Therefore, by the principle of induction,  _**P**_  holds when  _F_  has become a spanning tree, which is only possible if  _F_  is a minimum spanning tree itself.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTY4NjMxMTk4LDk3MzExODU0NCwtNDA0MD
gyMDgsOTc1MDA5Nzk0LDU3MzA1MjQ1NywtOTA3NzExMTg1LC0x
OTMyMTg1MDA1LC0yMTA4NzQxOTY5LDU5NjI0MzU4NCwtNzM3NT
M5NzAxLDY1NjQyMTIzOSwxNTU4Njg0MzUyLC0xMTE2ODg3OTEs
ODkzNzk5OTI3LDExMjU5OTA4MDcsLTM2OTA1OTc0OCw4MzQ3NT
I3MTYsLTU0Njg1NzYyNywxMDA4NzkxOTcyLDE0MTA1NjgyMjhd
fQ==
-->