...
모든 노드들이 연결되어 있고, 가중치가 주어진 그래프 $G$가 주어졌다고 하자. 알고리즘은 모든 간선을 하나씩 검토하고 그중에 기존에 신장트리에 간선을 추가했을때 cycle을 생성하는 경우를 제거한다. 그리고 그래프 G가 모든 노드들이 연결 될 수 있기 때문에 알고리즘에 의해서 나온 결과는 반드시 모든 노드와 이어지는 하나의 트리가 된다. 즉 spanningTree를 만족한다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTU0NTkyNzg2Niw1NzMwNTI0NTcsLTkwNz
