...
합성수 $n$을 $p*q$로 표현했을때, 둘 중 하나는 항상 $\sqrt n$이상, 다른 수는 $\sqrt{n}$ 이하이다. 이 특성을 이용하면 $n-1$까지가 아닌 $\sqrt{n}$까지만 순회하도록 최적화를 할 수 있다. 무슨 말인지 좀 더 풀어 보자면 ... 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTM0NzMwODU0LDc5MDkzOTMzNCwtNTUyNj
