...
합성수 $n$을 $p*q$로 표현했을때, 둘 중 하나는 항상 $\sqrt n$이상, 다른 수는 $\sqrt{n}$ 이하이다. 이 특성을 이용하면 $n-1$까지가 아닌 $\sqrt{n}$까지만 순회하면 소수인지 합성수인지 판별이 가능하다. 좀 더 자세히 풀어보자면 ...

만약 $p >= q(\sqrt{p} >=\sqrt{q})$라고 가정한다면 아래 식이 성립한다.
$$q <= \sqrt{p}\cdot\sqrt{q}$$$$p >= \sqrt{p}\cdot\sqrt{q}$$
정리하자면 p가 q보다 같거나 큰 상황에서 $\sqrt{p}\sqrt{q}$는 q와 같거나 크기 때문에 2에서 $\sqrt{p}\sqrt{q}$까지만 판별해보면 합성수인지 아닌지  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNjM5MTczNTQ5LDc5MDkzOTMzNCwtNTUyNj
MwNDQ5LC01NTI1MzY2MzAsNDI5NTg4MjI3LDgwMTAzMjk2OCw3
MzA5OTgxMTZdfQ==
-->